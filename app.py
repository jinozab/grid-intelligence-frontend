...
# ── Session state ──────────────────────────────────────────────────────────────
for key, val in [("prediction_data", None), ("backtest_data", None), ("backtest_days", 7)]:
...
if view == "Predict next 72 hours":
    ...
    if data:
        ...
        with m2:
            st.markdown(f"""<div class="metric-card {'danger' if max_val > threshold else ''}">
                {ICON_UP}
        ...
        with m4:
            st.markdown(f"""<div class="metric-card {'warning' if spike_hours > 0 else 'neutral'}">
                {ICON_SPIKE}
                <div class="metric-label">Spike Hours</div>
                <div class="metric-value">{spike_hours} <span class="metric-unit">h</span></div>
                <div class="metric-sub">&gt; {threshold} €/MWh</div>
            </div>""", unsafe_allow_html=True)
        ...
        fig = go.Figure()
        fig.add_hrect(y0=-200, y1=0,
            fillcolor="rgba(232,69,69,0.05)", line_width=0,
        ...
        fig.add_hline(y=threshold,
            line_dash="dash", line_color="rgba(232,69,69,0.4)", line_width=1,
        ...
        <table class="biz-table">
            <thead><tr>
                <th>Indicator</th><th>Value</th><th>Recommendation</th>
            </tr></thead>
            <tbody>
                <tr>
                    <td>⚡ Best Charge Window</td>
                    <td class="{'positive' if best_charge_price < 20 else ''}">{best_charge_ts} – {best_charge_end} · {best_charge_price:.1f} €/MWh</td>
                    <td>Optimal 4h battery / pump storage charging window</td>
                </tr>
                <tr>
                    <td>💰 Best Discharge Window</td>
                    ...
                    <td>Maximize revenue from stored energy</td>
                </tr>
                <tr>
                    <td>📉 Cheapest 4h Block</td>
                    <td class="positive">{best_charge_ts} – {best_charge_end}</td>
                    <td>Schedule energy-heavy industrial operations here</td>
                </tr>
                <tr>
                    <td>📈 Most Expensive 2h Block</td>
                    <td class="negative">{best_discharge_ts} – {best_discharge_end}</td>
                    <td>Reduce consumption · activate demand response</td>
                </tr>
                <tr>
                    <td>⚠️ Spike Risk</td>
                    <td class="{'negative' if spike_hours > 0 else 'positive'}">{spike_hours}h above {threshold} €/MWh</td>
                    <td>{"Activate hedging contracts or curtailment" if spike_hours > 0 else "No spike risk in window ✓"}</td>
                </tr>
                <tr>
                    <td>🟢 Negative Price Hours</td>
                    <td class="{'positive' if neg_hours > 0 else ''}">{neg_hours}h below 0 €/MWh</td>
                    <td>{"Maximize flexible load — grid pays you to consume" if neg_hours > 0 else "No negative prices expected"}</td>
                </tr>
                <tr>
                    <td>📊 Price Volatility</td>
                    <td class="{'negative' if std_val > 40 else 'warning'}">± {std_val:.1f} €/MWh</td>
                    <td>{"High volatility — active trading recommended" if std_val > 40 else "Moderate volatility — standard hedging sufficient"}</td>
                </tr>
            </tbody>
        </table>
        """, unsafe_allow_html=True)
